...
/*   Updated: 2025/09/04 17:45:42 by rpedrosa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    calculate_variables(t_data *data, t_enemy_draw *tmp, t_enemy *current)
{
	...
	tmp->inv = 1.0 / (data->plane_x * data->dir_vec_y - data->dir_vec_x * data->plane_y);
	tmp->trans_x = tmp->inv * (data->dir_vec_y * tmp->sprite_x - data->dir_vec_x * tmp->sprite_y);
	tmp->trans_y = tmp->inv * (-data->plane_y * tmp->sprite_x + data->plane_x * tmp->sprite_y);
	tmp->sprite_screen_x = (int)((SCREENWIDTH / 2) * (1 + tmp->trans_x / tmp->trans_y));
...
void    mount_image(t_data *data, t_enemy_draw *tmp)
{
	...
	while(++(tmp->y) < data->draw->end)
	{
		...
		if (tmp->color != 0xFFFFFF)
			my_mlx_pixel_put(data->draw->img_buffer, tmp->vertical, tmp->y, tmp->color);
	}
}